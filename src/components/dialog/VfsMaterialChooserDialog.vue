...
async function loadFiles() {
  ...
  try {
    ...
    list = list.filter(item => {
      return item.type === 'FOLDER' || item.name.lastIndexOf('.material.json') === item.name.length - 14;
    })
    list = list.sort((a, b) => {
    ...
    <div class="vfs-listview horizontal">
      ...
      >
        <div class="file-icon">
          ...
          <span v-else-if="item.thumbnail"><img :src="item.thumbnail" alt="thumbnail" style="width:140px; height:140px; object-fit:cover; border-radius:4px;" /></span>
          <span v-else>📄</span>
        </div>
