...
<template>
  <div v-if="materialFormRef" class="property-pane">
    <el-form v-if="materialFormRef" label-width="100px">
      <!-- 介质厚度 -->
      <el-form-item label="介质厚度">
        <el-slider
          v-model="materialFormRef.thickness"
          :min="0"
          :max="10"
          :step="0.01"
          @change="setMaterial({ thickness: materialFormRef.thickness })"
          class="range-input"
          size="small"
        />
        <span class="range-value">{{ materialFormRef.thickness?.toFixed(2) }}</span>
      </el-form-item>
      <!-- 介质厚度贴图 -->
      <el-form-item label="介质厚度贴图">
        <TexturePropertyItem
          :texture="materialProp.thicknessMap"
          @select-texture="onTextureSelected($event, 'thicknessMap')"
          @clear-texture="clearTexture('thicknessMap')"
          @change-texture="changeTexture('thicknessMap', $event)"
        />
      </el-form-item>
    </el-form>
  </div>
