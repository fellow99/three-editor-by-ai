...
<template>
  <div v-if="materialFormRef" class="property-pane">
    <el-form v-if="materialFormRef" label-width="100px">
      <!-- 透射率 -->
      <!--
        transmission: 透射率，范围0~1，表示材质允许多少光线透过。0为完全不透光，1为完全透明（但不影响颜色/折射等）。
      -->
      <el-form-item label="透射率">
        <el-slider
          v-model="materialFormRef.transmission"
          :min="0"
          :max="1"
          :step="0.01"
          @change="setMaterial({ transmission: materialFormRef.transmission })"
          class="range-input"
          size="small"
        />
        <span class="range-value">{{ materialFormRef.transmission?.toFixed(2) }}</span>
      </el-form-item>
      <!-- 透射率贴图 -->
      <!--
        transmissionMap: 透射率贴图，灰度图控制每个像素的透射率。白色区域完全透光，黑色区域完全不透光。
      -->
      <el-form-item label="透射率贴图">
        <TexturePropertyItem :texture="materialProp.transmissionMap"
          @select-texture="onTextureSelected($event, 'transmissionMap')" @clear-texture="clearTexture('transmissionMap')" 
          @change-texture="changeTexture('transmissionMap', $event)" />
      </el-form-item>
    </el-form>
  </div>
</template>
