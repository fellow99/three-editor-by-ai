...
<template>
  <div v-if="materialFormRef" class="property-pane">
    <el-form v-if="materialFormRef" label-width="100px">
      <!-- 位移贴图 -->
      <el-form-item label="位移贴图">
        <TexturePropertyItem
          :texture="materialProp.displacementMap"
          @select-texture="onTextureSelected($event, 'displacementMap')"
          @clear-texture="clearTexture('displacementMap')"
          @change-texture="changeTexture('displacementMap', $event)"
        />
      </el-form-item>
      <!-- 位移强度 -->
      <el-form-item label="位移强度">
        <el-slider
          v-model="materialFormRef.displacementScale"
          :min="0"
          :max="10"
          :step="0.01"
          @change="setMaterial({ displacementScale: materialFormRef.displacementScale })"
          class="range-input"
          size="small"
        />
        <span class="range-value">{{ materialFormRef.displacementScale?.toFixed(2) }}</span>
      </el-form-item>
      <!-- 位移偏移 -->
      <el-form-item label="位移偏移">
        <el-slider
          v-model="materialFormRef.displacementBias"
          :min="-5"
          :max="5"
          :step="0.01"
          @change="setMaterial({ displacementBias: materialFormRef.displacementBias })"
          class="range-input"
          size="small"
        />
        <span class="range-value">{{ materialFormRef.displacementBias?.toFixed(2) }}</span>
      </el-form-item>
    </el-form>
  </div>
</template>
