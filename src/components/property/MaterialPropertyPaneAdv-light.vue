...
<template>
  <div v-if="materialFormRef" class="property-pane">
    <el-form v-if="materialFormRef" label-width="100px">
      <!-- 光照贴图 -->
      <el-form-item label="光照贴图">
        <TexturePropertyItem
          :texture="materialProp.lightMap"
          @select-texture="onTextureSelected($event, 'lightMap')"
          @clear-texture="clearTexture('lightMap')"
          @change-texture="changeTexture('lightMap', $event)"
        />
      </el-form-item>
      <!-- 光照贴图强度 -->
      <el-form-item label="光照强度">
        <el-slider
          v-model="materialFormRef.lightMapIntensity"
          :min="0"
          :max="10"
          :step="0.01"
          @change="setMaterial({ lightMapIntensity: materialFormRef.lightMapIntensity })"
          class="range-input"
          size="small"
        />
        <span class="range-value">{{ materialFormRef.lightMapIntensity?.toFixed(2) }}</span>
      </el-form-item>
    </el-form>
  </div>
</template>
